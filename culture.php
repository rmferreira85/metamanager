...
if (isset($_GET['delete_id'])) {
    $id = $_GET['delete_id'];
    $sql = "DELETE FROM planCult WHERE id = ?";
...
if (isset($_POST['submit'])) {
    ...
    $idCult = $_POST['idCult'];
    $idInf = $_POST['modalFatInf']; 

    $query = mysqli_prepare($link, "INSERT INTO planCult (nome, idCult, idInf, Descr, idPlan, dataIn) VALUES (?, ?, ?, ?, ?, NOW())");
    mysqli_stmt_bind_param($query, "siisi", $nome, $idCult, $idInf, $comment, $id);
...
if (isset($_POST['editSubmit'])) {
    ...
    $idInf = $_POST['editFatInf']; 
    $id = $_POST['editFactorId'];

    $query = mysqli_prepare($link, "UPDATE planCult SET nome=?, idInf=?, Descr=? WHERE id=?");
    mysqli_stmt_bind_param($query, "sisi", $nome, $idInf, $comment, $id);
    if (mysqli_stmt_execute($query)) {
        $successSave = "Edição realizada com sucesso!";
        header("Location: culture.php?id=" . $_GET['id'] . "&success=1");
        exit();
    } else {
...
<body> 
    ...
    <div id="newPlanModal" class="modal" style="display:none;">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <br><br>
                <label for="modalFatInf">Influência:</label>
                <select name="modalFatInf" id="modalFatInf" class='select'>
                    <?php
                        $inf = mysqli_query($link, "SELECT * FROM typeInf");
                ...
                <br><br>
                <input type="hidden" id="idCult" name="idCult" value="">
    ...
    <div class="modal-content">
        ...
        <form method="POST">
            ...
            <br><br>
            <label for="editFatInf">Influência:</label>
            <select name="editFatInf" id="editFatInf" class='select'>
                <?php
                $inf = mysqli_query($link, "SELECT * FROM typeInf");
                while ($i = mysqli_fetch_array($inf)){
                    $selected = ($i['id'] == $factor_data['idInf']) ? 'selected' : '';
                ?>
                <option value="<?php echo $i['id']?>" <?php echo $selected ?>><?php echo $i['nome']?></option>
                <?php } ?>
            </select>
            <br><br>
    ...
    <div class="plans">
        ...
        <div class="sidebar">
            ...
            <a href="export_table_csv.php?table=planCult&id=<?php echo $_GET['id']; ?>" class="a_home">Exportar Tabela para CSV</a>
            <a href="export_table_xml.php?table=planCult&id=<?php echo $_GET['id']; ?>" class="a_home">Exportar Tabela para XML</a>
        </div>

        <div class="content-box">
            <?php
            $queryTipo = "SELECT * FROM typeFat WHERE id >= 14 AND id < 17";
            ...
            while ($rowTipo = mysqli_fetch_assoc($resultTipo)) {
                ...
                echo "<th scope='col'>Influência</th>";
                echo "<th scope='col'>...</th>";
                echo "</tr>";
                echo "</thead>";
                echo "<tbody>";

                $queryFat = "SELECT plancult.idInf, plancult.Descr, plancult.id, plancult.nome, typeInf.nome AS influencia_nome
                    FROM plancult INNER JOIN typeInf ON planCult.idInf = typeInf.id WHERE idCult = $idTipo AND idPlan = $id";
                ...
                if (mysqli_num_rows($resultFat) > 0) {
                    while ($factor_data = mysqli_fetch_assoc($resultFat)) {
                        ...
                        echo "<td>" . $factor_data['influencia_nome'] . "</td>";
                        echo "<td class='actions-cell'>";
                            echo "<a title='Edit Factor' class='btn btn-primary btn-sm view-factor-button' href='#' 
                                    ...
                                    data-influence='" . $factor_data['idInf'] . "' 
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            newPlanModalLinks.forEach(function(link) {
                link.onclick = function() {
                    ...
                    document.getElementById('idCult').value = typeId;
            ...
            function openEditModal(factorId, factorName, factorInfluence, factorDescr) {
                document.getElementById('editFactorId').value = factorId;
                document.getElementById('editFatName').value = factorName;
                document.getElementById('editFatInf').value = factorInfluence;
            ...
            viewFactorButtons.forEach(function(button) {
                button.addEventListener('click', function() {
                    ...
                    var factorInfluence = button.getAttribute('data-influence');
                    var factorDescr = button.getAttribute('data-descr');
                    openEditModal(factorId, factorName, factorInfluence, factorDescr);
                });
            });
            ...
            $('.delete-factor-button').click(function() {
                ...
                if (confirm('Você tem certeza que deseja excluir este fator?')) {
                    window.location.href = "culture.php?id=<?php echo $_GET['id']; ?>&delete_id=" + id;
                }
            });
