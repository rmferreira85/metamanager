...
if (isset($_GET['delete_id'])) {
    $id = $_GET['delete_id'];
    $sql = "DELETE FROM fatExt WHERE id = ?";
...
if (isset($_POST['submit'])) {
    ...
    $query = mysqli_prepare($link, "INSERT INTO fatExt (nome, idFat, idInf, Descr, idPlan, dataIn) VALUES (?, ?, ?, ?, ?, NOW())");
    mysqli_stmt_bind_param($query, "siisi", $nome, $idFat, $idInf, $comment, $id);
...
if (isset($_POST['editSubmit'])) {
    ...
    $query = mysqli_prepare($link, "UPDATE fatExt SET nome=?, idInf=?, Descr=? WHERE id=?");
    mysqli_stmt_bind_param($query, "sisi", $nome, $idInf, $comment, $id);
    if (mysqli_stmt_execute($query)) {
        $successSave = "Edição realizada com sucesso!";
        header("Location: fatEx.php?id=" . $_GET['id'] . "&success=1");
        exit();
    } else {
...
<body>
    ...
    <div class="modal-content">
        ...
        <div class="titleFat">
                    ...
                    <h3 class = "nomePage">Fatores Externos</h3>    
            </div>
            ...
            <a href="export_table_csv.php?table=fatExt&id=<?php echo $_GET['id']; ?>" class="a_home">Exportar Tabela para CSV</a>
            <a href="export_table_xml.php?table=fatExt&id=<?php echo $_GET['id']; ?>" class="a_home">Exportar Tabela para XML</a>
        </div>
            <div class="content-box">
                <?php
                $queryTipo = "SELECT * FROM typeFat WHERE id <= 4";
                ...
                while ($rowTipo = mysqli_fetch_assoc($resultTipo)) {
                    ...
                    $queryFat = "SELECT fatExt.idInf, fatExt.Descr, fatExt.id, fatExt.nome, typeInf.nome AS influencia_nome 
                                FROM fatExt INNER JOIN typeInf ON fatExt.idInf = typeInf.id WHERE fatExt.idFat = $idTipo AND fatExt.idPlan = $id";
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $('.delete-factor-button').click(function() {
                ...
                if (confirm('Você tem certeza que deseja excluir este fator?')) {
                    window.location.href = "fatEx.php?id=<?php echo $_GET['id']; ?>&delete_id=" + id;
                }
            });
